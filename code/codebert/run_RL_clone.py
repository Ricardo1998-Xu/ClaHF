...
from model import CloneModel as Model
...
def compute_rewards(args, codes1, codes2, predictions, reward_model, tokenizer, label_map):
    ...
    for code1, code2, pred_text in zip(codes1, codes2, predictions_text):
        ...
        input_ids = [tokenizer.cls_token_id] + code1_ids + [tokenizer.sep_token_id] + code2_ids + pred_ids + [tokenizer.sep_token_id]
...
def train(args, train_dataset, model, ref_model, reward_model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):
            ...
            rewards = compute_rewards(args, batch[2], batch[3], actions, reward_model, tokenizer, label_map)
...
def main():
    ...
    # codebert-base-Scanpy_1.9.0_remove_target-FL-lr0.0000001
    ...
    reference_model.load_state_dict(
        torch.load(os.path.join(args.sft_path, "checkpoint-best-acc/model.bin"), map_location=device))
    ...
    reward_model.load_state_dict(
        torch.load(os.path.join(args.reward_path, "checkpoint-best-acc/model.bin"), map_location=device))
    reward_model.eval()

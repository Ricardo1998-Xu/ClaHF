...
from transformers import (OPTConfig, AutoTokenizer, OPTModel, Qwen3Model, Qwen3Config,
...
from RewardDataset import RewardPairDataset as RewardDataset
...
MODEL_CLASSES = {
    ...
    'qwen3': (Qwen3Config, Qwen3Model, AutoTokenizer)
}
...
def reward_dataset_collate_fn(batch):
    """
    batch: List[List[Dict]]，长度=batch_size，内部每条为一个 rank list
    返回：
        input_ids: (batch_size, rank_list_len, block_size)
        attention_mask: 同上
    """
    batch_input_ids = []
    ...
    return {
        "input_ids": batch_input_ids,               # List[Tensor]，长度=batch_size
...
def evaluate(args, model, tokenizer):
    ...
    for rank_rewards in all_rank_rewards:
        ...
        if top1_index == 0:  # 因为真实偏好排序中，正确的 candidate 是排在第0位的
            correct_top1 += 1
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, inputs in enumerate(bar):
            batch_rank_rewards = []
            for batch_idx in range(len(inputs['input_ids'])):
                input_ids = inputs['input_ids'][batch_idx].to(args.device)  # [num_label, seq_len]
                attention_mask = inputs['attention_mask'][batch_idx].to(args.device)
                model.train()
                rewards = model(input_ids=input_ids, attention_mask=attention_mask)
                batch_rank_rewards.append(rewards)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and (step + 1) % args.save_steps == 0:
                    ...
                    if results['top1_acc'] > best_acc:
                        best_acc = results['top1_acc']
                        logger.info("  " + "*" * 20)
...
def main():
    ...
    parser.add_argument("--train_data_file", default='../reward_dataset/MRPC/train.jsonl', type=str,
                        help="The input training data file (a text file).")
    parser.add_argument("--eval_data_file", default='../reward_dataset/MRPC/valid.jsonl', type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")

    parser.add_argument("--output_dir", default='./weights/three/MRPC/Reward-ccrl-alpha0.0-lr0.00001-seed123456', type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument('--runs_path', type=str,
                        default='./runs/three/MRPC/Reward-ccrl-alpha0.0-lr0.00001-seed123456', help='save runs path')

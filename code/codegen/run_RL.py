...
from transformers import (OPTConfig, OPTForSequenceClassification, AutoTokenizer, OPTModel,CodeGenConfig, CodeGenModel,
...
MODEL_CLASSES = {
    'codegen': (CodeGenConfig, CodeGenModel, CodeGenModel, AutoTokenizer)
}
...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.label = label
        self.code1_text = code1_text
        self.code2_text = code2_text
...
def convert_examples_to_features(js,tokenizer,args):
    ...
    return InputFeatures(source_tokens, source_ids, js['label'], code1, code2)
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):
        return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label), self.examples[i].code1_text, self.examples[i].code2_text
...
def compute_rewards(args, inputs, predictions, reward_model, tokenizer, label_map):
    ...
    def verbalize(pred, label_map):
        ...
        return f"Prediction: the code is {label_str}"
...
def compute_rewards_clone(args, codes1, codes2, predictions, reward_model, tokenizer, label_map):
    def verbalize(pred, label_map):
        label_str = label_map.get(str(pred), f"class {pred}")
        return f"Prediction: the two code snippets are {label_str}"
    ...
    for code1, code2, pred_text in zip(codes1, codes2, predictions_text):
        pred_tokens = tokenizer.tokenize(middle + pred_text)
        pred_ids = tokenizer.convert_tokens_to_ids(pred_tokens)
        max_text_len = args.block_size - len(pred_ids) - 3  # [CLS] 和 [SEP]
        half = max_text_len // 2
        code1_tokens = tokenizer.tokenize(prefix + code1)[:half]
        code2_tokens = tokenizer.tokenize(prefix + code2)[:half]
        code1_ids = tokenizer.convert_tokens_to_ids(code1_tokens)
        code2_ids = tokenizer.convert_tokens_to_ids(code2_tokens)
        input_ids = [tokenizer.cls_token_id] + code1_ids + [tokenizer.sep_token_id] + code2_ids + pred_ids + [
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):
            ...
            with torch.no_grad():
                ...
                old_log_probs = torch.log(ref_probs + 1e-8)
            ...
            rewards = compute_rewards_clone(args, batch[2], batch[3], actions, reward_model, tokenizer, label_map)
            ...
            loss = ppo_loss + args.vf_coef * value_loss
        ...
        tb_writer.add_scalar("val_mcc", results['eval_mcc'], idx)
...
def main():
    ...
    parser.add_argument('--num_labels', type=int, default=2,
                        help='class num')
    parser.add_argument("--json_path", default='../BCB.json', type=str, )
    parser.add_argument("--train_data_file", default='../dataset/BigCloneBench/train.jsonl', type=str,
                        help="The input training data file (a text file).")
    parser.add_argument("--eval_data_file", default='../dataset/BigCloneBench/valid.jsonl', type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")

    parser.add_argument("--sft_path", default='', type=str,
                        help="")
    parser.add_argument("--reward_path", default='', type=str,
                        help="")
    ...
    reference_model = DecoderClassifier(reference_model, config, tokenizer, args)

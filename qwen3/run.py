...
from transformers import (WEIGHTS_NAME, get_linear_schedule_with_warmup,Qwen3Config, Qwen3ForSequenceClassification,
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        preds = np.argmax(train_logits, axis=1)
        train_acc = np.mean(train_labels == preds)
        train_mcc = matthews_corrcoef(train_labels, preds)
        ...
        tb_writer.add_scalar("train_mcc", train_mcc, idx)
        ...
        # model_to_save = model.module if hasattr(model, 'module') else model
        # output_dir = os.path.join(args.output_dir, "model_{}.bin".format(idx))
        # torch.save(model_to_save.state_dict(), output_dir)
...
def main():
    ...
    parser.add_argument("--output_dir", default='./weights/three/MRPC/CE-lr0.00002-seed123456', type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument('--runs_path', type=str,
                        default='./runs/three/MRPC/CE-lr0.00002-seed123456',
                        help='save runs path')

...
from transformers import (T5Config, T5ForConditionalGeneration, T5Tokenizer, RobertaTokenizer, AutoTokenizer,
...
MODEL_CLASSES = {
    'codet5+': (T5Config, T5ForConditionalGeneration, T5ForConditionalGeneration, AutoTokenizer)
}
...
def compute_rewards(args, inputs, predictions, reward_model, tokenizer, label_map):
    ...
    def verbalize(pred, label_map):
        ...
        return f"Prediction: the code {label_str}"
...
def main():
    ...
    parser.add_argument("--sft_path", type=str, help="",
                        default='')
    ...
    parser.add_argument("--return_logits", default=False, action='store_true',
                        help="Whether to return logits.")
    ...
    reference_model = DefectModel(reference_model, config, tokenizer, args)
    ...
    reward_model = RewardModel(reward_model, config, args)

...
class CloneModel(nn.Module):
    def __init__(self, encoder, config, tokenizer, args):
        ...
        self.classifier = nn.Linear(self.encoder.config.hidden_size, args.num_labels)  # 输出二分类
        self.value_head = nn.Linear(self.encoder.config.hidden_size, 1)
        # Define dropout layer, dropout_probability is taken from args.
        self.dropout = nn.Dropout(args.dropout_probability)
    ...
    def forward(self, input_ids=None, labels=None):
        ...
        cls_logits = hidden_states[:, 0, :]  # (B, H)

        logits = self.classifier(cls_logits)  # (batch_size, 2)
        values = self.value_head(cls_logits).squeeze(-1)

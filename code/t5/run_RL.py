...
MODEL_CLASSES = {
    't5': (T5Config, T5ForConditionalGeneration, T5ForConditionalGeneration, T5Tokenizer)
}
...
def compute_rewards(args, inputs, predictions, reward_model, tokenizer, label_map):
    ...
    def verbalize(pred, label_map):
        ...
        return f"Prediction: the sentence is grammatically {label_str}"
...
def compute_rewards_pair(args, texts1, texts2, predictions, reward_model, tokenizer, label_map):
    def verbalize(pred, label_map):
        label_str = label_map.get(str(pred), f"class {pred}")
    ...
    tokenizer.cls_token = '<s>'
    tokenizer.sep_token = '</s>'
    for text1, text2, pred_text in zip(texts1, texts2, predictions_text):
        pred_tokens = tokenizer.tokenize(middle + pred_text)
        pred_ids = tokenizer.convert_tokens_to_ids(pred_tokens)
        max_text_len = args.block_size - len(pred_ids) - 3  # [CLS] 和 [SEP]
        half = max_text_len // 2
        text1_tokens = tokenizer.tokenize(prefix + text1)[:half]
        text2_tokens = tokenizer.tokenize(prefix + text2)[:half]
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default='./code/t5-base', type=str,
                        help="The model checkpoint for weights initialization.")
    parser.add_argument("--tokenizer_name", default="./code/t5-base", type=str,

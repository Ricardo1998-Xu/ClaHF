...
MODEL_CLASSES = {
    'qwen': (AutoConfig, AutoModelForSequenceClassification, AutoTokenizer),
    'qwen3': (Qwen3Config, Qwen3ForSequenceClassification, AutoTokenizer),
}
...
class ConfusionMatrix(object):
    ...
    def summary(self, results_path, probs=None, labels_all=None, n_bins=10):
        ...
        if probs is not None and labels_all is not None:
            confidences = np.max(probs, axis=1)  # 每个样本的置信度
            predictions = np.argmax(probs, axis=1)  # 预测类别
...
def test(args, model, tokenizer):
    ...
    mcc = matthews_corrcoef(labels, logits)
    ...
    if args.num_labels == 2:
        ...
        for i in range(args.num_labels):  # 计算每一个标签的假正例率（fpr）和真正例率（tpr）
        ...
        plt.plot(fpr_dict["macro"], tpr_dict["macro"], color='midnightblue',  # 画关于正面的roc曲面
...
def main():
    ...
    parser.add_argument('--num_labels', type=int, default=2,
                        help='class num')
    parser.add_argument("--test_data_file", default='../dataset/sst2/test.jsonl', type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")

    parser.add_argument("--output_dir", default='./weights/three/sst2/PPO-ccrl-alpha0.0-vf0.0-lr0.000001-seed123456', type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument('--results_path', type=str,
                        default='./results/three/sst2/PPO-ccrl-alpha0.0-vf0.0-lr0.000001-seed123456', help='save results path')
